...
const checkCategoryHandler = async (req, res) => {
  if (req.body) {
    const user = await searchOne(req.body, modelName);
    if (user) {
      return res
        .status(200)
        .send({ status: "success", message: "Sub Category found" });
    }
  }
  return res
    .status(200)
    .send({ status: "error", message: "Sub Category not found" });
};
...
router.post("/check", checkCategoryHandler);
