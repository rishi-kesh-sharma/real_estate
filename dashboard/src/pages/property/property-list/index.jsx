...
import { count, search, remove } from '../property-service';
...
const DeleteButton = (props) => {
  ...
  const showDeleteConfirm = (property) => {
    confirm({
      title: `Do you Want to delete ${property.name}?`,
      icon: <ExclamationCircleOutlined />,
      content: `${property.name} will be deleted permanently.`,
      okText: 'Yes',
      okType: 'danger',
      cancelText: 'No',
      onOk: async () => {
        console.log('OK');
        const r = await remove(property._id);
        if (r.success) {
  ...
  const columns = [
    {
      ...
      render: (dom, entity) => {
        return (
          <a
            onClick={() => {
              history.push(`/properties/edit/${entity._id}`);
            }}
          >
            {dom}
          </a>
